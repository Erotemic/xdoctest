...
def test_strip_ansi_codes():
    ...
    Example:
        >>> from xdoctest.runner import _strip_ansi_codes
...
def test_format_want_lines():
    """
    Test want line formatting.

    Example:
        >>> from xdoctest.runner import _format_want_lines
...
def test_detect_indentation(tmp_path):
    """
    Test indentation detection from source files.

    Example:
        >>> import xdoctest
        >>> from xdoctest.runner import _detect_indentation
        ...
        ...     test_file = f.name
        >>> # Parse it
        >>> examples = list(xdoctest.core.parse_doctestables(test_file, style='auto'))
        >>> example = examples[0]
        >>> example._parse()
        >>> part = example._parts[0]
        >>> # Detect indentation
        >>> indent = _detect_indentation(example, part)
        >>> len(indent)  # Should be 8 spaces (4 for function + 4 for docstring)
        8
        >>> # Clean up
        >>> import os
        >>> os.unlink(test_file)
